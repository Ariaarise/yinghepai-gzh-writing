...
> **注意**：金句里默认用石墨下划线（`border-bottom:2px solid #52525B`）标记关键词。若要更强的品牌黄下划线点睛（`border-bottom:2px solid #FFFB00`），计入全篇 ≤3 处品牌黄配额，慎重使用。

---

## 组件 3 前言导读区域（本文看点，三列极简线框目录卡）

> 3 个及以上章节时生成。上方极细线 + 极浅灰底，石墨灰编号 + 深炭标题，纯灰阶无品牌黄，大留白。展示**精选 3 个核心看点**，不是全量章节列表。
...
> **核心理念**：几乎全灰阶，品牌黄只在真正关键的锚点出现（全篇 ≤3 处总配额，含引言卡）。
>
> **优先级**：① 7d 石墨下划线（正文默认标记，每段都应考虑）→ ② 7a 普通/深炭加粗为主 → ③ 7b 极浅灰底深炭字标签（每篇 2~4 个）→ ④ 7c 极浅灰背景（次要）→ ⑤ 7e 荧光笔（偶尔长句强调）→ ⑥ 7f 品牌黄下划线（全篇最强锚点，总计 ≤3 处含引言卡）
...
品牌黄加粗（仅限极少数关键锚点，全篇 ≤3 处总配额）：

```html
<strong style="color:#FFFB00;"><span leaf="">品牌黄加粗锚点</span></strong>
```
...
### 7f. 品牌黄下划线（点睛锚点，全篇 ≤3 处含引言卡）

```html
<span style="border-bottom:2px solid #FFFB00;font-weight:700;color:#27272A;"><span leaf="">品牌黄下划线点睛词</span></span>
```
...
### 9b. 品牌黄竖条提示条（强调，全篇 ≤3 处配额共享）

> 品牌黄竖条 + 深炭文字，无背景色，是品牌黄在正文中最自然的出现方式。

```html
<section style="border-left:3px solid #FFFB00;padding:14px 0 14px 22px;margin:0 10px 24px;">
  <p style="font-size:14px;font-weight:700;color:#27272A;margin:0;line-height:1.8;">
...
品牌黄描边（点睛标签，全篇 ≤3 处配额共享）：

```html
<span style="display:inline-block;border:1px solid #FFFB00;color:#FFFB00;font-size:11px;font-weight:600;padding:2px 10px;border-radius:2px;margin-right:6px;letter-spacing:0.5px;"><span leaf="">核心标签</span></span>
```
...
### 16b. 右侧署名区（硬核派团队署名，右对齐多行）

> 用于文章末尾落款：撰稿 / 指导 / 编辑 / 审核等，**右对齐**，辅助灰色小字，行间留白。字段按实际参与人员填写，没有的字段整行删掉。适合社区公众号（硬核派⚡️）的团队署名习惯。

```html
<section style="padding:0 10px 30px;text-align:right;">
  <section style="padding-top:24px;">
    <p style="margin:0 0 6px;font-size:12px;color:#A1A1AA;letter-spacing:1px;line-height:1.8;">
      <span leaf="">撰稿：{{撰稿人}}</span>
    </p>
    <p style="margin:0 0 6px;font-size:12px;color:#A1A1AA;letter-spacing:1px;line-height:1.8;">
      <span leaf="">指导：{{指导人}}</span>
    </p>
    <p style="margin:0;font-size:12px;color:#A1A1AA;letter-spacing:1px;line-height:1.8;">
      <span leaf="">编辑：{{编辑人}}</span>
    </p>
  </section>
</section>
```

> **使用说明**：署名字段（撰稿/指导/编辑/审核/统筹等）按文章实际情况选填，没有的整行删除；全部字段未知时整块删除，不写占位。团队署名一般放在组件 15 END 线之后、正文最末尾。
...
| 层级 | 样式 | 用途 | 频率 |
|------|------|------|------|
| **锚点层** | 品牌黄下划线 7f / 品牌黄竖条 9b / 品牌黄加粗 7a（`#FFFB00`） | 全文最关键锚点 | 全篇 ≤3 处（含引言卡） |
| **标记层** | 石墨下划线 7d（默认）+ 字重 600 / 极浅灰底标签 7b | 正文关键词强调 | 每段 1~3 处 |
| **容器层** | 石墨竖条引用 8x / 提示 9x / 标签组 10 / 卡片 11-12 | 引用、旁注、提示、结构化信息 | 按需 |

**克制原则**：
- 品牌黄（`#FFFB00`）全篇 ≤3 处，所有品牌黄使用共享此配额（引言卡橙下划线、正文橙竖条、橙标签各算一处）
- 石墨下划线是默认正文标记，高频使用
...
| `**加粗文字**` | 组件 7a 深炭加粗（默认）/ 品牌黄加粗（锚点 ≤3） | 深炭加粗为主 |
...
| 注意/警告 | 组件 9a 石墨提示 / 9b 品牌黄提示 / 9c 踩坑提示 | |
